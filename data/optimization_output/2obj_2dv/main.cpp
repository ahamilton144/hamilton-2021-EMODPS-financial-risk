...
/* main.cpp - Evolutionary Multi-Objective Direct Policy Search for adaptive financial risk management for a hydropower producer.
...
// Constants to be changed depending on run type & parameters
#define DPS_RUN_TYPE 0  // 0: 2dv version; 1: full DPS with RBFs, maxDebt formulation; 2: full DPS with RBFs, minRev formulation
#define BORG_RUN_TYPE 2
#define RUNTIME_OUTPUT 100 // how many runtime snapshots to print (only used in parallel cluster, BORG_RUN_TYPE==2)
#define NUM_YEARS 20    //20yr sims
#define NUM_SAMPLES 50000
#define NUM_OBJECTIVES 2
#define NUM_RBF 2              // number of radial basis functions, used if DPS_RUN_TYPE>0
...
#define NUM_PARAM_SAMPLES 151  // number of LHC samples in param file. If SENSITIVITY_ANALYSIS==0, will only use last line, the values for SFPUC, Oct 2016.
...
#define NUM_VARIABLES_STOCHASTIC_INPUT 3  //3 cols in input: revenue, snow contract net payout, power price index
...
ublas::vector<double> annualized_cashflow(NUM_SAMPLES); // objectives/constraints
ublas::vector<double> hedge_frequency(NUM_SAMPLES);
ublas::vector<double> max_fund_balance(NUM_SAMPLES);
#if DPS_RUN_TYPE < 2
ublas::vector<double> debt_steal(NUM_SAMPLES);
#else
ublas::vector<double> min_cashflow(NUM_SAMPLES);
#endif

ublas::vector<double> revenue(NUM_YEARS);                   // stochastic drivers
ublas::vector<double> unit_payout_snow_contract(NUM_YEARS); // snow contract net payout
ublas::vector<double> power_price_index(NUM_YEARS + 1);     // power price index

ublas::vector<double> adjusted_revenue(NUM_YEARS); // state variables
...
ublas::vector<double> snow_contract_slope(NUM_YEARS);
...
void portfolioProblem(double *problem_dv, double *problem_objs, double *problem_constraints)
{
    ...
    zero(hedge_frequency);
    zero(max_fund_balance);
    zero(revenue);
    zero(power_price_index);
    zero(unit_payout_snow_contract);
...
#if DPS_RUN_TYPE > 0
    // get dvs
    for (int i = 0; i < int(dv_d.size()); i++)
    {
        dv_d(i) = problem_dv[i]; // threshold variables
    }
    for (int i = 0; i < int(dv_c.size()); i++)
    {
        dv_c(i) = problem_dv[i + dv_d.size()]; // centers for RBFs
    }
    for (int i = 0; i < int(dv_b.size()); i++)
    {
        dv_b(i) = max(EPS, problem_dv[i + dv_d.size() + dv_c.size()]); // radii for RBFs
    }
    for (int i = 0; i < int(dv_w.size()); i++)
    {
        dv_w(i) = problem_dv[i + dv_d.size() + dv_c.size() + dv_b.size()]; // weights for RBFs
    }
    for (int i = 0; i < int(dv_a.size()); i++)
    {
    ...
    double fixed_snow_contract_slope = policySnowContractValue_2dv(problem_dv[1]);
#endif
    ...
    double total_payout_snow_contract = 0.;
    double cash_in = 0.;
    ...
    for (int s = 0; s < NUM_SAMPLES; s++)
    {
        ...
        for (int i = 0; i < NUM_YEARS; i++)
        {
            ...
            unit_payout_snow_contract(i) = stochastic_input[index + i][INDEX_STOCHASTIC_SNOW_PAYOUT];
            power_price_index(i + 1) = stochastic_input[index + i][INDEX_STOCHASTIC_POWER_INDEX];
            //            printf("%f  %f  %f\n", revenue(i), unit_payout_snow_contract(i), payout_power_contract(i));
        }

        // initial simulation variables
        zero(snow_contract_slope);
        zero(fund_balance);
        ...
        hedge_frequency(s) = 0;
#if DPS_RUN_TYPE == 0
        if (fixed_snow_contract_slope > EPS)
        {
            hedge_frequency(s) = 1;
        }
#endif
        ...
        for (int i = 0; i < NUM_YEARS; i++)
        {
            ...
            // find next policy-derived index insurance and reserve fund withdrawal

#if DPS_RUN_TYPE > 0
            snow_contract_slope(i) = policySnowContractValue(fund_balance(i), debt(i), power_price_index(i),
                                                             dv_d, dv_c, dv_b, dv_w, dv_a);
            if (abs(snow_contract_slope(i)) > EPS)
            {
                hedge_frequency(s) = 1;
            }
            total_payout_snow_contract = snow_contract_slope(i) * (unit_payout_snow_contract(i) - lambda_prem_shift);
            cash_in = revenue(i) + total_payout_snow_contract - (debt(i) * interest_debt);
...
#else
            total_payout_snow_contract = fixed_snow_contract_slope * (unit_payout_snow_contract(i) - lambda_prem_shift);
            cash_in = revenue(i) + total_payout_snow_contract - (debt(i) * interest_debt);
            ...
            annualized_cashflow(s) += adjusted_revenue(i) * discount_factor(i);
        }
...
#if DPS_RUN_TYPE < 2
        debt_q95(vmax(debt));                                  // for q95(max(debt)) objective
        debt_steal(s) = debt(NUM_YEARS) - debt(NUM_YEARS - 1); // for constraint to ensure that debt use is "sustainable"
#else
        min_cashflow(s) = vmin(adjusted_revenue);
#endif
    ...
    problem_objs[0] = -1 * vsum(annualized_cashflow) / NUM_SAMPLES; // max: average annualized adjusted_revenue, across samp
#if DPS_RUN_TYPE < 2
    problem_objs[1] = accumulator::quantile(debt_q95, accumulator::quantile_probability = 0.95); //minimize 95th percentile of max debt
#else
    problem_objs[1] = -1 * vsum(min_cashflow) / NUM_SAMPLES;
#endif
#if NUM_OBJECTIVES > 2
    problem_objs[2] = 1 * vsum(hedge_frequency) / NUM_SAMPLES;  // min: avg_avg_hedging complexity
    problem_objs[3] = 1 * vsum(max_fund_balance) / NUM_SAMPLES; // min: max_fund_balance
#endif

#if DPS_RUN_TYPE < 2
    // check constraint
    problem_constraints[0] = max(0.0, (vsum(debt_steal) / NUM_SAMPLES) - EPS_CONS1); // constraint to ensure that debt use is "sustainable"
#endif

    //    printf("\n\n\n%f   %f   %f   %f   %f\n\n\n", problem_objs[0], problem_objs[1], problem_objs[2], problem_objs[3], problem_constraints[0]);

    annualized_cashflow.clear();
    hedge_frequency.clear();
    max_fund_balance.clear();
    fund_balance.clear();
    revenue.clear();
    power_price_index.clear();
    unit_payout_snow_contract.clear();
    discount_factor.clear();
    snow_contract_slope.clear();
                        ...
                        const ublas::vector<double> &f_dv_w, const ublas::vector<double> &f_dv_a)
{
    ...
    {
#if SHARED_RBFS == 0
        arg_exp = -pow(f_fund_balance * USEINRBF_FUND_WITHDRAWAL / NORMALIZE_FUND - f_dv_c((decision_order * NUM_INPUTS_RBF * NUM_RBF) + NUM_INPUTS_RBF * i), 2) / pow(f_dv_b((decision_order * NUM_INPUTS_RBF * NUM_RBF) + NUM_INPUTS_RBF * i), 2) - pow(f_debt * USEINRBF_DEBT_WITHDRAWAL / NORMALIZE_FUND - f_dv_c((decision_order * NUM_INPUTS_RBF * NUM_RBF) + NUM_INPUTS_RBF * i + 1), 2) / pow(f_dv_b((decision_order * NUM_INPUTS_RBF * NUM_RBF) + NUM_INPUTS_RBF * i + 1), 2) - pow(f_power_price_index * USEINRBF_POWER_WITHDRAWAL / NORMALIZE_POWER_PRICE - f_dv_c((decision_order * NUM_INPUTS_RBF * NUM_RBF) + NUM_INPUTS_RBF * i + 2), 2) / pow(f_dv_b((decision_order * NUM_INPUTS_RBF * NUM_RBF) + NUM_INPUTS_RBF * i + 2), 2) - pow((f_cash_in * USEINRBF_CASHIN_WITHDRAWAL + NORMALIZE_REVENUE) / (2 * NORMALIZE_REVENUE) - f_dv_c((decision_order * NUM_INPUTS_RBF * NUM_RBF) + NUM_INPUTS_RBF * i + 3), 2) / pow(f_dv_b((decision_order * NUM_INPUTS_RBF * NUM_RBF) + NUM_INPUTS_RBF * i + 3), 2);
        cash_out += f_dv_w(decision_order * NUM_RBF + i) * exp(arg_exp);

#elif SHARED_RBFS == 1
        arg_exp = -pow(f_fund_balance * USEINRBF_FUND_WITHDRAWAL / NORMALIZE_FUND - f_dv_c(NUM_INPUTS_RBF * i), 2) / pow(f_dv_b(NUM_INPUTS_RBF * i), 2) - pow(f_debt * USEINRBF_DEBT_WITHDRAWAL / NORMALIZE_FUND - f_dv_c(NUM_INPUTS_RBF * i + 1), 2) / pow(f_dv_b(NUM_INPUTS_RBF * i + 1), 2) - pow(f_power_price_index * USEINRBF_POWER_WITHDRAWAL / NORMALIZE_POWER_PRICE - f_dv_c(NUM_INPUTS_RBF * i + 2), 2) / pow(f_dv_b(NUM_INPUTS_RBF * i + 2), 2) - pow((f_cash_in * USEINRBF_CASHIN_WITHDRAWAL + NORMALIZE_REVENUE) / (2 * NORMALIZE_REVENUE) - f_dv_c(NUM_INPUTS_RBF * i + 3), 2) / pow(f_dv_b(NUM_INPUTS_RBF * i + 3), 2);
        cash_out += f_dv_w(decision_order * NUM_RBF + i) * exp(arg_exp);
                               ...
                               const ublas::vector<double> &f_dv_w, const ublas::vector<double> &f_dv_a)
{
    ...
    {
#if SHARED_RBFS == 0
        arg_exp = -pow(f_fund_balance * USEINRBF_FUND_HEDGE / NORMALIZE_FUND - f_dv_c((decision_order * NUM_INPUTS_RBF * NUM_RBF) + NUM_INPUTS_RBF * i), 2) / pow(f_dv_b((decision_order * NUM_INPUTS_RBF * NUM_RBF) + NUM_INPUTS_RBF * i), 2) - pow(f_debt * USEINRBF_DEBT_HEDGE / NORMALIZE_FUND - f_dv_c((decision_order * NUM_INPUTS_RBF * NUM_RBF) + NUM_INPUTS_RBF * i + 1), 2) / pow(f_dv_b((decision_order * NUM_INPUTS_RBF * NUM_RBF) + NUM_INPUTS_RBF * i + 1), 2) - pow(f_power_price_index * USEINRBF_POWER_HEDGE / NORMALIZE_POWER_PRICE - f_dv_c((decision_order * NUM_INPUTS_RBF * NUM_RBF) + NUM_INPUTS_RBF * i + 2), 2) / pow(f_dv_b((decision_order * NUM_INPUTS_RBF * NUM_RBF) + NUM_INPUTS_RBF * i + 2), 2);
#elif SHARED_RBFS == 1
        arg_exp = -pow(f_fund_balance * USEINRBF_FUND_HEDGE / NORMALIZE_FUND - f_dv_c(NUM_INPUTS_RBF * i), 2) / pow(f_dv_b(NUM_INPUTS_RBF * i), 2) - pow(f_debt * USEINRBF_DEBT_HEDGE / NORMALIZE_FUND - f_dv_c(NUM_INPUTS_RBF * i + 1), 2) / pow(f_dv_b(NUM_INPUTS_RBF * i + 1), 2) - pow(f_power_price_index * USEINRBF_POWER_HEDGE / NORMALIZE_POWER_PRICE - f_dv_c(NUM_INPUTS_RBF * i + 2), 2) / pow(f_dv_b(NUM_INPUTS_RBF * i + 2), 2);
#elif SHARED_RBFS == 2
        arg_exp = -pow(f_fund_balance * USEINRBF_FUND_HEDGE / NORMALIZE_FUND - f_dv_c((decision_order * NUM_INPUTS_RBF * NUM_RBF) + NUM_INPUTS_RBF * i), 2) / pow(f_dv_b((decision_order * NUM_INPUTS_RBF * NUM_RBF) + NUM_INPUTS_RBF * i), 2) - pow(f_debt * USEINRBF_DEBT_HEDGE / NORMALIZE_FUND - f_dv_c((decision_order * NUM_INPUTS_RBF * NUM_RBF) + NUM_INPUTS_RBF * i + 1), 2) / pow(f_dv_b((decision_order * NUM_INPUTS_RBF * NUM_RBF) + NUM_INPUTS_RBF * i + 1), 2) - pow(f_power_price_index * USEINRBF_POWER_HEDGE / NORMALIZE_POWER_PRICE - f_dv_c((decision_order * NUM_INPUTS_RBF * NUM_RBF) + NUM_INPUTS_RBF * i + 2), 2) / pow(f_dv_b((decision_order * NUM_INPUTS_RBF * NUM_RBF) + NUM_INPUTS_RBF * i + 2), 2);
#endif
...
double policyWithdrawal_2dv(const double f_fund_balance, const double f_cash_in, const double f_max_fund_size)
{
    double withdrawal = 0.;
    if (f_cash_in < -EPS)
    { // negative cash_in, want to make a withdrawal
        if (f_fund_balance < EPS)
        { // is there non-zero fund for withdrawal?
            withdrawal = 0.;
        }
        else
        {
    ...
    { // positive cash_in, want to deposit
        if (f_fund_balance > (f_max_fund_size - EPS))
        {                                                  // is fund balance larger than allowed max?
...
int main(int argc, char *argv[])
{
    ...
    FILE *myfile;
    string dir_synth = read_directory;
    myfile = fopen(dir_synth.append("synthetic_data.txt").c_str(), "r");
    ...
    if (myfile == NULL)
    {
        perror("Error opening synthetic data file \n");
    }
    else
    {
    ...
    myfile2 = fopen(dir_param.append("param_LHC_sample_withLamPremShift.txt").c_str(), "r");
    ...
    else
    {
        ...
        while ((fgets(buffer2, BUFFER_MAX_SIZE, myfile2) != NULL) & (linenum < NUM_PARAM_SAMPLES))
        {
            linenum++;
            if (buffer2[0] != '#')
            {
    ...
    BORG_Algorithm_output_frequency(NFE / RUNTIME_OUTPUT);
#endif
    ...
    for (int p = p_min; p < p_max; ++p)
    {
        ...
        BORG_Problem_set_epsilon(problem, 0, EPS_ANNREV);        // avg_annualized_cashflow (units $M, so 0.01=$10,000)
#if DPS_RUN_TYPE < 2
...
#endif
#if NUM_OBJECTIVES > 2
        BORG_Problem_set_epsilon(problem, 2, EPS_MAXCOMPLEXITY); // hedge_frequency (unitless)
        ...
        sprintf(output_filename, "%ssets/DPS_param%d_seedS%d_seedB%d.set", write_directory_cstr, p, seed_sample, seed_borg);
        ...
        strcpy(write_directory_cstr, write_directory.c_str());
        sprintf(output_filename, "%ssets/DPS_param%d_seedS%d_seedB%d.set", write_directory_cstr, p, seed_sample, seed_borg);
        sprintf(runtime, "%sruntime/DPS_param%d_seedS%d_seedB%d.runtime", write_directory_cstr, p, seed_sample, seed_borg);
        delete[] write_directory_cstr;
        ...
        //printf("Param run %d finished, %f seconds, Output to %s\n", p, elapsed_secs, output_filename);
    }
#endif
