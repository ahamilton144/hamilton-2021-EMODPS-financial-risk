...
#define NUM_VARIABLES_STOCHASTIC_INPUT 9            //6 cols in input: swe,powIndex,revRetail,revWholesale,sswp,pswp, sweCorr0.25,sweCorr0.50,sweCorr0.75
...
#define INDEX_STOCHASTIC_SWECORR 8
...
#define SHARED_RBFS 2     // 0 = 1 rbf shared between hedge and withdrawal policies. 1 = separate rbf for each. 2 = rbf for hedge, and 2dv formulation for withdrawal.
#if SHARED_RBFS==2
...
#if NUM_INPUTS_RBF > 4
            swe_correlate(i) = stochastic_input[index + i][INDEX_STOCHASTIC_SWECORR];
#endif
...
#if NUM_INPUTS_RBF > 4
        arg_exp += - pow(f_swe_correlate / NORMALIZE_SWE - f_dv_c((decision_order * NUM_INPUTS_RBF * NUM_RBF) + NUM_INPUTS_RBF * i +4), 2)
                         / pow(f_dv_b((decision_order * NUM_INPUTS_RBF * NUM_RBF) + NUM_INPUTS_RBF * i + 4), 2);
#endif
...
#if NUM_INPUTS_RBF > 4
        arg_exp += - pow(f_swe_correlate / NORMALIZE_SWE - f_dv_c(NUM_INPUTS_RBF * i +4), 2)
                         / pow(f_dv_b(NUM_INPUTS_RBF * i + 4), 2);
#endif
        cash_out += f_dv_w(decision_order * NUM_RBF + i) * exp(arg_exp) ;
    ...
    for (int i = 0; i < NUM_RBF; i++){
        ...
        arg_exp = -pow(f_fund_balance * USEINRBF_FUND_HEDGE / NORMALIZE_FUND - f_dv_c((decision_order * NUM_INPUTS_RBF * NUM_RBF) + NUM_INPUTS_RBF * i), 2)
                  ...
                  - pow(f_power_price_index * USEINRBF_POWER_HEDGE / NORMALIZE_POWER_PRICE - f_dv_c((decision_order * NUM_INPUTS_RBF * NUM_RBF) + NUM_INPUTS_RBF * i + 2), 2)
                         / pow(f_dv_b((decision_order * NUM_INPUTS_RBF * NUM_RBF) + NUM_INPUTS_RBF * i + 2), 2);
#if NUM_INPUTS_RBF > 3
        arg_exp += - pow(f_swe_correlate / NORMALIZE_SWE - f_dv_c((decision_order * NUM_INPUTS_RBF * NUM_RBF) + NUM_INPUTS_RBF * i + 3), 2)
                         / pow(f_dv_b((decision_order * NUM_INPUTS_RBF * NUM_RBF) + NUM_INPUTS_RBF * i + 3), 2);
#endif
#endif
...
double policyWithdrawal_2dv(const double f_fund_balance, const double f_cash_in, const double f_max_fund_size) {
    ...
    if (f_cash_in < -EPS){  // negative cash_in, want to make a withdrawal
        ...
        if (f_fund_balance > (f_max_fund_size - EPS)){  // is fund balance larger than allowed max?
            ...
            withdrawal = min(-(f_max_fund_size - f_fund_balance), f_cash_in);   // else deposit up to max, or whole positive cash flow, whichever is smaller
        }
    }
    return withdrawal;
}
...
int main(int argc, char* argv[]) {
    ...
    retest_file = argv[5];      // directory where "sets" and "runtime" folders are located, for writing output
#endif
    ...
    myfile = fopen(dir_HHsamp.append("HHsamp10132019.txt").c_str(), "r");
...
#if DPS_RUN_TYPE==1
        sprintf(output_filename, "%ssets/PortDPS_DPS_maxDebt_samp%d_seedS%d_seedB%d.set", write_directory_cstr, NUM_SAMPLES, seed_sample, seed_borg);
#elif DPS_RUN_TYPE==2
        sprintf(output_filename, "%ssets/PortDPS_DPS_minRev_samp%d_seedS%d_seedB%d.set", write_directory_cstr, NUM_SAMPLES, seed_sample, seed_borg);
#else
        sprintf(output_filename, "%ssets/PortDPS_DPS_2dv_samp%d_seedS%d_seedB%d.set", write_directory_cstr, NUM_SAMPLES, seed_sample, seed_borg);
#endif
...
#if DPS_RUN_TYPE==1
        sprintf(output_filename, "%ssets/PortDPS_DPS_maxDebt_samp%d_seedS%d_seedB%d.set", write_directory_cstr, NUM_SAMPLES, seed_sample, seed_borg);
        sprintf(runtime, "%sruntime/PortDPS_DPS_maxDebt_samp%d_seedS%d_seedB%d.runtime", write_directory_cstr, NUM_SAMPLES, seed_sample, seed_borg);
#elif DPS_RUN_TYPE==2
        sprintf(output_filename, "%ssets/PortDPS_DPS_minRev_samp%d_seedS%d_seedB%d.set", write_directory_cstr, NUM_SAMPLES, seed_sample, seed_borg);
        sprintf(runtime, "%sruntime/PortDPS_DPS_minRev_samp%d_seedS%d_seedB%d.runtime", write_directory_cstr, NUM_SAMPLES, seed_sample, seed_borg);
#else
        sprintf(output_filename, "%ssets/PortDPS_DPS_2dv_samp%d_seedS%d_seedB%d.set", write_directory_cstr, NUM_SAMPLES, seed_sample, seed_borg);
        sprintf(runtime, "%sruntime/PortDPS_DPS_2dv_samp%d_seedS%d_seedB%d.runtime", write_directory_cstr, NUM_SAMPLES, seed_sample, seed_borg);
#endif
        delete[] write_directory_cstr;
