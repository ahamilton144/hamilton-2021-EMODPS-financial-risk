...
def empirical_copula_many(many, empirical_ranks):
  ...
  for i in range(len(w)):
    w.iloc[i] = empirical_copula_point([many[0].iloc[i], many[1].iloc[i]], empirical_ranks)
  return(w)

def plot_empirical_synthetic_copula_swe(dir_figs, swe):
    np.random.seed(1)
    shp_g_danFeb, dum, scl_g_danFeb = gamma.fit(swe.danFeb, floc=0)
    shp_g_danApr, dum, scl_g_danApr = gamma.fit(swe.danApr, floc=0)
    # sample from gammas using copulas
    ...
    # empirical copula w
    w = empirical_copula_many([RFeb, RApr], [RFeb, RApr])
    nw = len(w)
    h = (nw * w - 1) / (nw - 1)
    ...
    samp_fitted = multivariate_normal.rvs(mean=np.array([0, 0]), size=nw * ncop,
                                          cov=[[1, corr_norm_equiv],
                                               [corr_norm_equiv, 1]])
    u = norm.cdf(samp_fitted)
    samp_fitted = [pd.Series(u[:, 0]), pd.Series(u[:, 1])]

    samp_uncorr = multivariate_normal.rvs(mean=np.array([0, 0]), size=nw * ncop,
                                          cov=[[1, 0.0],
                                               [0.0, 1]])
    u = norm.cdf(samp_uncorr)
    samp_uncorr = [pd.Series(u[:, 0]), pd.Series(u[:, 1])]

    samp_corr = multivariate_normal.rvs(mean=np.array([0, 0]), size=nw * ncop,
                                        cov=[[1, 1],
                                             [1, 1]])
    u = norm.cdf(samp_corr)
    samp_corr = [pd.Series(u[:, 0]), pd.Series(u[:, 1])]

    copula_data_fitted = empirical_copula_many([RFeb, RApr], samp_fitted).sort_values().values
    copula_fitted_fitted = np.zeros([ncop, nw])
    copula_uncorr_uncorr = np.zeros([ncop, nw])
    copula_corr_corr = np.zeros([ncop, nw])

    for i in range(ncop):
      copula_fitted_fitted[i, :] = (empirical_copula_many([samp_fitted[0].iloc[(i * nw):((i + 1) * nw)],
                                                           samp_fitted[1].iloc[(i * nw):((i + 1) * nw)]],
                                                          samp_fitted).sort_values().values)
      copula_uncorr_uncorr[i, :] = (empirical_copula_many([samp_uncorr[0].iloc[(i * nw):((i + 1) * nw)],
                                                           samp_uncorr[1].iloc[(i * nw):((i + 1) * nw)]],
                                                          samp_uncorr).sort_values().values)
      copula_corr_corr[i, :] = (empirical_copula_many([samp_corr[0].iloc[(i * nw):((i + 1) * nw)],
                                                       samp_corr[1].iloc[(i * nw):((i + 1) * nw)]],
                                                      samp_corr).sort_values().values)
