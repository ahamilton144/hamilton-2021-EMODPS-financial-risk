...
def get_moea_output(dir_generated_inputs, dir_moea_output, p_sfpuc, meanRevenue, minSnowContract, minMaxFund, debt_filter = True):
  ...
  for i in range(p_sfpuc+1):
    if (i == p_sfpuc):
      fil = dir_moea_output + 'seedsBase/DPS_2dv_seedsBase_borg_retest.resultfile'
    else:
      fil = dir_moea_output + 'seedsSensitivity/DPS_2dv_seedsSensitivity_param' + str(i) + '_borg_retest.resultfile'
    ...
    dum['p'] = param_list.p.iloc[i]
    dum['c'] = param_list.c.iloc[i]
    dum['delta'] = param_list.delta.iloc[i]
    dum['Delta_fund'] = param_list.Delta_fund.iloc[i]
    dum['Delta_debt'] = param_list.Delta_debt.iloc[i]
    dum['lam'] = param_list.lam.iloc[i]
    dum['lam_prem_shift'] = param_list.lam_prem_shift.iloc[i]
...
def plot_pareto_baseline(dir_figs, moea_solns, p_sfpuc, cases_sfpuc_index):
  ...
  plt.annotate('C', xy=(moea_solns.exp_ann_cashflow_retest.iloc[cases_sfpuc_index[2]]-0.02,
                 moea_solns.q95_max_debt_retest.iloc[cases_sfpuc_index[2]]-2.7))
...
def plot_tradeoff_cloud(dir_figs, moea_solns, meanRevenue, p_sfpuc, debt_filter):
  ...
  # plt.ylabel(r"$\leftarrow$ $\hat{J}^{debt}$ (\$M)")
  # plt.xlabel(r"$\hat{J}^{cash} (\$M/year)\rightarrow$")
...
### outputs dataframe ####
# ##########################################################################
def get_metrics_single(metricfile, hvfile, p, s):
  # read data
...
def get_metrics_all(dir_moea_output, p_sfpuc, nSeedsBase, nSeedsSensitivity):
  ...
  for s in range(1, nSeedsBase+1):
    metric_file = dir_moea_output + 'seedsBase/metrics/PortDPS_2dv_param' + str(p) + '_samp50000_seed1_seedB' +\
                  str(s) + '_borg.metrics'
    hv_file = dir_moea_output + 'seedsBase/DPS_2dv_seedsBase_borg.hypervolume'
    metrics_seedsBase.append(get_metrics_single(metric_file, hv_file, p, s))

  ### now do same for sensitivity analysis samples
  metrics_nSeedsSensitivity = []
  p_successes = []
  for p in range(p_sfpuc):
    for s in range(1, nSeedsSensitivity+1):
      try:
        metric_file = dir_moea_output + 'nSeedsSensitivity/metrics/PortDPS_2dv_param' + str(p) + \
                      '_samp50000_seed1_seedB' + str(s) + '_borg.metrics'
        hv_file = dir_moea_output + 'nSeedsSensitivity/DPS_2dv_nSeedsSensitivity_param' + str(p) + '_borg.hypervolume'
        metrics_nSeedsSensitivity.append(get_metrics_single(metric_file,
                                                           hv_file, p, s))
        if (s == 1):
  ...
  return(metrics_seedsBase, metrics_nSeedsSensitivity, p_successes)
...
### outputs plot, no return. ####
# ##########################################################################
def plot_hypervolume(dir_figs, metrics_seedsBase, metrics_nSeedsSensitivity, p_successes, nSeedsBase, nSeedsSensitivity, nfe):
  ...
  for j,p in enumerate(param_samps):
    ...
    for s in range(1, 11):
      i = 10 * j + s - 1
      hv = metrics_nSeedsSensitivity[i]['Hypervolume']
      ax.plot(function_eval / 1000, hv, c=cmap_vir(col_scal[s - 1]), alpha=0.7)
    ax.set_ylim([-0.1, 1.15])
    if (rj == 3)&(cj == 1):
...
### outputs plot, no return. ####
# ##########################################################################
def plot_generational_distance(dir_figs, metrics_seedsBase, metrics_nSeedsSensitivity, p_successes, nSeedsBase, nSeedsSensitivity, nfe):
  ...
  for j,p in enumerate(param_samps):
    ...
    for s in range(1, (nSeedsSensitivity+1)):
      i = 10 * j + s - 1
      hv = metrics_nSeedsSensitivity[i]['GenerationalDistance']
      ax.plot(function_eval / 1000, hv, c=cmap_vir(col_scal[s - 1]), alpha=0.7)
    ax.set_ylim([-0.01, 0.11])
    if (rj == 3)&(cj == 1):
      ax.set_xlabel('Thousands of Function Evaluations')
...
### outputs plot, no return. ####
# ##########################################################################
def plot_epsilon_indicator(dir_figs, metrics_seedsBase, metrics_nSeedsSensitivity, p_successes, nSeedsBase, nSeedsSensitivity, nfe):
  col_scal = np.arange(1,(nSeedsBase+1))/(nSeedsBase+1)
  cmap_vir = cm.get_cmap('viridis')
  function_eval = np.arange(0, nfe+1, 200)
  fig = plt.figure()
  ...
  for j,p in enumerate(param_samps):
    ...
    for s in range(1, (nSeedsSensitivity+1)):
      i = 10 * j + s - 1
      hv = metrics_nSeedsSensitivity[i]['EpsilonIndicator']
      ax.plot(function_eval / 1000, hv, c=cmap_vir(col_scal[s - 1]), alpha=0.7)
    ax.set_ylim([-0.05, 0.55])
    if (rj == 3)&(cj == 1):
      ax.set_xlabel('Thousands of Function Evaluations')
